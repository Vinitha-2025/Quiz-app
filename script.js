...
const showQuizResult=() =>{
    ...
    const resultText=`You answered <b>${correctAnswersCount}</b> out <b>${numberOfQuestions}</b> Questions correctly. Great effect!`;
...
const getRandomQuestion = () => {
    ...
    (cat => cat.category.toLowerCase() === quizCategory.toLowerCase()).questions || [];
//Shoiw the results if all questions have been did
...
const highLightCorrectAnswer=()=>{
    const correctOption=answerOptions.querySelectorAll(".answer-option")[currentQuestion.correctAnswer-1];
...
const handleAnswer=(option, answerIndex)=>{
    clearInterval(timer);
    const isCorrect=(currentQuestion.correctAnswer-1) === answerIndex;
...
//Start the quiz and return the random question
const startQuiz=()=>{
